...
author:
  - name: Florent Baty
    ...
    email: \email{florent.baty@kssg.ch}
    url:
  - name: Christian Ritz
    affiliation: University of Copenhagen
    # To add another line, use \AND at the end of the previous one as above
  - name: Sandrine Charles
    affiliation: University of Lyon \AND 
...
title:
  formatted: "A Toolbox for Nonlinear Regression in \\proglang{R}:  \nThe Package \\pkg{nlstools}"
  # If you use tex in the formatted title, also supply version without
  plain:     "A Toolbox for Nonlinear Regression in R:\\ The Package nlstools"
  # For running headers, if needed
  short:     "The Package \\pkg{nlstools}"
abstract: 
...
keywords:
  ...
  formatted: [Confidence regions, residuals, diagnostic tools, resampling techniques, starting values, 6-minute walk test]
  plain:     [nonlinear regression, diagnostic tools, R]
preamble: >
  \usepackage{amsmath}
...
# Introduction
  ...
  Therefore nonlinear regression may appear to be more daunting than linear regression. It requires a higher degree of user interaction, both for initializing the estimation procedure and for interpreting the results. The package \pkg{nlstools} offers tools for addressing these steps when fitting nonlinear regression models using \code{nls()} (function implemented in the \proglang{R} package \pkg{stats}). \pkg{nlstools} is available on the Comprehensive \proglang{R} Archive Network at \url{http://CRAN.R-project.org/package=nlstools}. 
...
The function \code{nlsBoot()} uses non-parametric bootstrap of mean centered residuals to obtain a given number (argument \code{niter}) of bootstrap estimates \citep[Chapter~8]{venables&ripley:2002}. Bootstrap estimates, standard errors together with median and percentile confidence intervals (2.5\% and 97.5\% percentiles of bootstrapped estimates) \citep[pp.~225-226]{venables&ripley:2002} are displayed by the generic function \code{summary()}. The associated plotting function can be used both for a pairwise representation of the bootstrap estimates or, as shown in Figure\ \ref{fig:resampling} (right panel), for a boxplot representation of the distribution of each bootstrapped parameter. 
